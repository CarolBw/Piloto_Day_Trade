...
def carregar_dados(df: pd.DataFrame, db_path: str):
    ...
    for _, row in df.iterrows():
        ...
        # 1. Inserção na tabela dim_tempo
        cursor.execute("""
            INSERT INTO dim_tempo (id_tempo, data, hora, dia_da_semana_entrada)
            VALUES (?, ?, ?, ?)
        """, (id_tempo, row['data'], row['hora'], row['dia_da_semana_entrada']))

        # 2. Inserção na tabela dim_indicadores
        cursor.execute("""
            INSERT INTO dim_indicadores (
                id_indicadores, id_tempo, SMA_10, EMA_10, MACD, Signal_Line, rsi, OBV,
                retorno, volatilidade, fechamento_dia, volume_dia, maximo_dia, minimo_dia
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            id_tempo, id_tempo,
            row['SMA_10'], row['EMA_10'], row['MACD'], row['Signal_Line'],
            row['rsi'], row['OBV'], row['retorno'], row['volatilidade'],
            row.get('fechamento_dia', None), row.get('volume_dia', None),
            row.get('maximo_dia', None), row.get('minimo_dia', None)
        ))

        # 3. Inserção na tabela dim_lags
        cursor.execute("""
        ...
        # 4. Inserção na tabela dim_operacional
        cursor.execute("""
            INSERT INTO dim_operacional (
                id_operacional, id_tempo,
                dia_da_semana_previsao, hora_num, minuto, mercado_aberto,
                fechamento_dia_anterior, volume_dia_anterior,
                maximo_dia_anterior, minimo_dia_anterior
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            id_tempo, id_tempo,
        ...
        # 5. Inserção na tabela fato_precos
        cursor.execute("""
            INSERT INTO fato_precos (
                id_fato_precos, id_tempo,
                abertura, minimo, maximo, fechamento,
                fechamento_dia, volume_dia, maximo_dia, minimo_dia
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            id_tempo, id_tempo,
            row['abertura'], row['minimo'], row['maximo'], row['fechamento'],
            row.get('fechamento_dia', None), row.get('volume_dia', None),
            row.get('maximo_dia', None), row.get('minimo_dia', None)
        ))
