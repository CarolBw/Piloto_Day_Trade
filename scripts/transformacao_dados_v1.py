...
def calcular_indicadores(df):
    ...
    df['retorno'] = df['preco_fechamento'].pct_change().fillna(0)
    df['volatilidade'] = df['retorno'].rolling(20).std().fillna(0)

    df['SMA_10'] = df['preco_fechamento'].rolling(10).mean().fillna(0)
    df['EMA_10'] = df['preco_fechamento'].ewm(span=10, adjust=False).mean()
    ...
    df['SMA_20'] = df['preco_fechamento'].rolling(20).mean()
    df['std_dev'] = df['preco_fechamento'].rolling(20).std()
    df['upper_band'] = df['SMA_20'] + 2 * df['std_dev']
    df['lower_band'] = df['SMA_20'] - 2 * df['std_dev']

    df['MACD'] = df['preco_fechamento'].ewm(span=12).mean() - df['preco_fechamento'].ewm(span=26).mean()
    df['Signal_Line'] = df['MACD'].ewm(span=9).mean()

    df['OBV'] = (df['volume'] * np.sign(df['preco_fechamento'].diff())).fillna(0).cumsum()

    scaler = MinMaxScaler()
    df[['fechamento_normalizado', 'volume_normalizado']] = scaler.fit_transform(df[['preco_fechamento', 'volume']])
